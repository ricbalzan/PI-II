...
class PermissionController extends Controller
{
    ...
    public function create(Request $request)
    {
        ...
        try{
            ...
            if($permission){ 
                return redirect('permission')->with('success', 'Permissão criada com sucesso!');
            }else{
                return redirect('permission')->with('error', 'Falha ao criar a Permissão! Tente novamente.');
            }
        }catch (\Exception $e) {
            $bug = $e->getMessage();
            return redirect()->back()->with('error', $bug);
    ...
    public function delete($id)
    {
        ...
        if($permission){
            ...
            return redirect('permission')->with('success', 'Permissao excluída!');
        }else{
            return redirect('404');
        }
    }
    ...
    public function getPermissionBadgeByRole(Request $request){
        ...
        if($role->name == 'Super Admin'){
            $badges = 'Super Admin tem todas as permissões!';
        }
